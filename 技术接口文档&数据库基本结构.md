...
# 二,企业端注册四步引导功能
  ...
  ## 3.数据库设计
    公司基本信息：company_profile(company_id PK, name, industry, country, city, size, homepage, description, status, created_at, updated_at)
    HR信息：company_contacts(contact_id PK, company_id FK, name, email, phone_country_code, phone_number, position, department, is_primary)
    面试邀约模版信息：invitation_templates(template_id PK, company_id FK, name, subject, body, language, created_at, updated_at)
    验证码信息：verification_tokens(token_id PK, company_id FK, code, channel, expires_at, consumed, created_at)
  ## 4.错误处理？
    比如：
    | 场景      | 返回码         | 描述            |
