...
- **进入面试房间**：`POST /api/candidate/interview-sessions/{jobCandidateId}/start`
  ...
  - 成功返回 `CandidateInterviewStartResponse`，包含 `interviewRecordId`、`aiSessionId`、题目列表与附带元数据；后端默认标记面试状态为 `IN_PROGRESS`。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L63-L68】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L251-L303】【F:src/main/java/com/example/grpcdemo/service/RestInterviewQuestionClient.java†L19-L86】
- **头像上传**：`POST /api/candidate/interview-sessions/{jobCandidateId}/profile-photo`，请求体 `CandidateInterviewProfilePhotoRequest` 携带 Base64 图片、文件名与 MIME；若未上传头像，`submitAnswer` 会直接返回 409 阻止答题。预览使用 `GET /profile-photo`。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L84-L103】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L341-L375】
- **题目作答与语音上传**：`POST /api/candidate/interview-sessions/{jobCandidateId}/answers`
  - 请求体 `CandidateInterviewAnswerRequest` 包含题号、音频 Base64、时长、转写文本及扩展 metadata。后端会自动生成音频 ID、覆盖旧音频并维护转写 JSON，然后返回 `CandidateInterviewAnswerResponse`（带最新面试记录及当前音频 DTO），方便第三张图同步刷新波形与字幕。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L70-L76】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L305-L356】
- **完成/放弃面试**：
