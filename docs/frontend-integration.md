...
- **左侧邀约列表**：`GET /api/candidate/interview-sessions/{jobCandidateId}/invitations`
  ...
  - 响应 `CandidateInterviewInvitationListResponse`，包含卡片数组 `invitations[]`（企业名称、岗位、HR 联系方式、面试状态、预检/头像完成度等）以及 `statusCounters[]`（顶部标签统计），`total` 和 `activeStatus` 直接用于渲染分页与高亮。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L32-L47】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L69-L154】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewInvitationItem.java†L9-L128】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewStatusCounter.java†L9-L29】
- **右侧详情与提醒**：`GET /api/candidate/interview-sessions/{jobCandidateId}` 返回 `CandidateInterviewDetailResponse`，包含最新面试记录快照、预检状态、技术检查项列表、头像上传状态以及 `answerDeadlineAt`（进入房间后 60 分钟的作答截止时间）；`readyForInterview=true` 时才允许进入面试房间。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L49-L54】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L156-L212】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewDetailResponse.java†L9-L63】
...
- **进入面试房间**：`POST /api/candidate/interview-sessions/{jobCandidateId}/start`
  ...
  - 返回 `CandidateInterviewStartResponse`，包含 `interviewRecordId`、`aiSessionId`、题目列表、附带元数据以及新增的 `answerDeadlineAt`。后端在第一次进入时写入 `interview_started_at`、计算 60 分钟截止时间并把候选人状态切换为 `IN_PROGRESS`；若已到截止时间会直接返回 410（`GONE`）。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L63-L68】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L251-L317】【F:src/main/java/com/example/grpcdemo/service/RestInterviewQuestionClient.java†L19-L86】
...
- **题目作答与语音上传**：`POST /api/candidate/interview-sessions/{jobCandidateId}/answers`
  - 请求体 `CandidateInterviewAnswerRequest` 需携带题号、音频 Base64、时长、前端通过语音转写得到的文本 `transcript` 以及扩展 metadata。后端会在 60 分钟窗口内校验是否超时、自动生成音频 ID、覆盖旧音频并维护转写 JSON，然后返回 `CandidateInterviewAnswerResponse`（带最新面试记录及当前音频 DTO），方便第三张图同步刷新波形与字幕。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L70-L76】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L319-L360】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewAnswerRequest.java†L16-L58】
- **完成/放弃面试**：
