...
- **麦克风/网络预检**：`POST /api/candidate/interview-sessions/{jobCandidateId}/precheck`
  ...
  - 后端写入 `candidate_interview_records.precheck_*` 字段并将候选人状态调整为 `SCHEDULED`，响应与详情一致用于刷新提示。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L56-L61】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L214-L249】【F:src/main/java/com/example/grpcdemo/entity/CandidateInterviewRecordEntity.java†L24-L108】
- **进入面试房间**：`POST /api/candidate/interview-sessions/{jobCandidateId}/start`
  ...
  - 返回 `CandidateInterviewStartResponse`，包含 `interviewRecordId`、`aiSessionId`、题目列表与元数据，首次进入会写入 `roomEnteredAt`，但不会立即开始计时，`interviewStartedAt` 与 `answerDeadlineAt` 可能为空；若记录已被判定超时将直接返回 410（`GONE`）。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L65-L71】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L271-L336】【F:src/main/java/com/example/grpcdemo/service/RestInterviewQuestionClient.java†L19-L86】
- **开始作答（点击“开始录音”）**：`POST /api/candidate/interview-sessions/{jobCandidateId}/begin-answering`
  - 无需请求体。后端会确认头像已上传且仍在答题窗口内，然后写入 `interview_started_at` 与 `answer_deadline_at=+60min`，并把候选人状态切换为 `IN_PROGRESS`。响应 `CandidateInterviewBeginResponse`，包含 `interviewRecordId`、`interviewStartedAt` 与 `answerDeadlineAt` 供倒计时使用。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L73-L77】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L338-L365】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewBeginResponse.java†L1-L29】
- **头像上传**：`POST /api/candidate/interview-sessions/{jobCandidateId}/profile-photo`，请求体 `CandidateInterviewProfilePhotoRequest` 携带 Base64 图片、文件名与 MIME。头像写入成功后 `CandidateInterviewProfilePhotoDto` 会返回下载地址；未上传头像时 `submitAnswer` 会直接返回 409 阻止作答。预览使用 `GET /profile-photo`。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L84-L103】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L333-L377】
- **题目作答与语音上传**：`POST /api/candidate/interview-sessions/{jobCandidateId}/answers`
  - 请求体 `CandidateInterviewAnswerRequest` 需携带题号、音频 Base64、时长、前端通过语音转写得到的文本 `transcript` 以及扩展 metadata。后端会先确认头像已上传并且已经执行过 `begin-answering`，随后在 60 分钟窗口内校验是否超时、自动生成音频 ID、覆盖旧音频并维护转写 JSON，然后返回 `CandidateInterviewAnswerResponse`（带最新面试记录及当前音频 DTO），方便第三张图同步刷新波形与字幕。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L79-L85】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L367-L420】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewAnswerRequest.java†L16-L58】
- **完成/放弃面试**：
  - 完成：`POST /api/candidate/interview-sessions/{jobCandidateId}/complete`（可附带总时长与 metadata）。在答题时限内调用会把状态设为 `COMPLETED`；若接口被调用时已经超时，后端会保留超时前的问答与音频、补齐结束时间并返回 `timedOut=true` 的 `CandidateInterviewRecordResponse`，前端应据此提示“作答已超时”。为了避免数据丢失，倒计时归零时也要自动调用一次该接口。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L78-L83】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L358-L405】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewRecordResponse.java†L9-L115】
  - 放弃：`POST /api/candidate/interview-sessions/{jobCandidateId}/abandon` 支持传入 `reason`，后端将候选人标记为 `ABANDONED` 并把原因写入记录 metadata，响应新的邀约卡片用于第五张图状态切换。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L85-L92】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L397-L428】
- **倒计时与强制结束**：候选人进入面试后若未在 `answerDeadlineAt` 之前上传全部答案，后端会自动把记录标记为 `TIMED_OUT`、补齐结束时间并在 `metadata.timeout` 中记录 `deadline`/`finalizedAt`。此时继续调用头像/作答接口会返回 410，但仍可调用 `complete` 获取已保存的问答快照给用户查看或提示重新预约。【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L251-L360】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L358-L405】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L586-L642】
- **音视频下载**：头像和题目音频分别通过 `GET /profile-photo` 与 `GET /audios/{audioId}` 提供二进制流，便于候选人或运营下载回放。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L93-L118】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L330-L353】
