...
- **获取面试记录**：`GET /api/enterprise/job-candidates/{jobCandidateId}/interview-record`
  - 返回 `CandidateInterviewRecordResponse`，字段含面试方式、起止时间、题目与问答列表、`audios[]`（音频片段，包含 `downloadUrl`/`durationSeconds` 等）、`positionId`、`positionName` 以及 `transcriptRaw`（AI 原始转写 JSON）；若不存在记录返回 404。前端点击“已面试”按钮后打开详情页使用该接口即可。【F:src/main/java/com/example/grpcdemo/controller/JobCandidateController.java†L41-L45】【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L322-L409】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewRecordResponse.java†L9-L95】
...
- **完成/放弃面试**：
  - 完成：`POST /api/candidate/interview-sessions/{jobCandidateId}/complete`（可附带总时长与 metadata），后端会计算结束时间、写入元数据并把状态设为 `COMPLETED`，响应 `CandidateInterviewRecordResponse` 供第四张图展示。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L78-L83】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L358-L395】
...
- **倒计时与强制结束**：候选人进入面试后若未在 `answerDeadlineAt` 之前上传全部答案，后端会自动把记录标记为 `TIMED_OUT`、补齐结束时间并拒绝后续的头像/答题/完成请求；前端可据此在 60 分钟到期时提示候选人重新预约面试。【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L251-L360】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L586-L642】
