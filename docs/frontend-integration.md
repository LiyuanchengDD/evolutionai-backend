...
- **批量上传**：`POST /api/enterprise/jobs/{positionId}/candidates/import`
  ...
  - 若未解析出邮箱，`inviteStatus=EMAIL_MISSING`，前端需提示补录；解析失败时 `importedCandidates[].resumeAvailable=true` 且 `summary.waitingInvite` 会同步增加，方便在“待邀约”标签上实时展示数量。【F:src/main/java/com/example/grpcdemo/controller/CompanyJobCandidateController.java†L20-L40】【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L60-L137】【F:src/main/java/com/example/grpcdemo/service/RestResumeParser.java†L16-L71】【F:src/main/java/com/example/grpcdemo/service/StubResumeParser.java†L13-L29】
- **岗位内检索 / 状态筛选 / 分页**：`GET /api/enterprise/jobs/{positionId}/candidates`
  - 查询参数：
    - `keyword` —— 同时按姓名 / 邮箱 / 电话模糊匹配，忽略大小写。
    - `status` —— 五个固定标签之一：`WAITING_INVITE`（待邀约）、`NOT_INTERVIEWED`（未面试）、`INTERVIEWED`（已面试）、`DROPPED`（已放弃）或 `ALL`（全部）。缺省时等同 `ALL`。
    - `page` & `pageSize` —— 从 0 开始的页码与每页数量（默认 `page=0`、`pageSize=20`，最大 200），配合前端滚动分页使用。
  - 响应 `JobCandidateListResponse`，除原有 `candidates` 外包含 `summary.waitingInvite`、`summary.notInterviewed`、`summary.interviewed`、`summary.dropped`、`summary.all` 以及分页字段 `total`、`page`、`pageSize`、`hasMore`、`nextPage`；同时新增 `summary.statusCounts{}`（`JobCandidateListStatus` → 数量），前端可按枚举直接渲染顶部 5 个标签并在切换时无须再次计算。后端始终按导入时间倒序返回数据。【F:src/main/java/com/example/grpcdemo/controller/CompanyJobCandidateController.java†L37-L63】【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L207-L282】【F:src/main/java/com/example/grpcdemo/controller/dto/JobCandidateListResponse.java†L9-L63】【F:src/main/java/com/example/grpcdemo/controller/dto/JobCandidateStatusSummary.java†L9-L69】
- **候选人详情**：`GET /api/enterprise/job-candidates/{jobCandidateId}/resume`
  - 返回 `JobCandidateResumeResponse`，包含基础信息、AI 解析出的 HTML 片段以及原始简历的 `<iframe>` HTML，可直接嵌入预览区。新增字段：`positionName`、`interviewRecordAvailable`、`aiEvaluationAvailable`、`interviewCompletedAt`、`interviewDeadlineAt`，用于在详情页顶部显示岗位名称并控制“查看面试记录/AI 报告”按钮及倒计时。【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L168-L229】【F:src/main/java/com/example/grpcdemo/controller/JobCandidateController.java†L24-L28】【F:src/main/java/com/example/grpcdemo/controller/dto/JobCandidateResumeResponse.java†L16-L86】
...
- **获取面试记录**：`GET /api/enterprise/job-candidates/{jobCandidateId}/interview-record`
  - 返回 `CandidateInterviewRecordResponse`，字段含面试方式、起止时间、答题截止时间 `answerDeadlineAt`、题目与问答列表、`audios[]`（音频片段，包含 `downloadUrl`/`durationSeconds` 等）、`positionId`、`positionName`、`timedOut` 标记以及 `transcriptRaw`（AI 原始转写 JSON）；若不存在记录返回 404。前端点击“已面试”按钮后打开详情页使用该接口即可。【F:src/main/java/com/example/grpcdemo/controller/JobCandidateController.java†L41-L45】【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L322-L409】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewRecordResponse.java†L9-L115】
- **下载面试音频**：`GET /api/enterprise/job-candidates/{jobCandidateId}/interview-record/audios/{audioId}`
  - 返回二进制音频流，响应头附带文件名与 `Content-Type`，可直接交给 `<audio>` 或下载保存；若音频不存在返回 404。【F:src/main/java/com/example/grpcdemo/controller/JobCandidateController.java†L47-L66】【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L229-L255】
- **写入/更新面试记录**：`PUT /api/enterprise/job-candidates/{jobCandidateId}/interview-record`
  - 请求体 `CandidateInterviewRecordRequest` 除题目数组 `questions[]` 与 `metadata` 外，新增 `audios[]`（`audioId`、`questionSequence`、`contentType`、`durationSeconds`、`transcript`、`base64Content`），用于一次性上传或覆盖候选人答题音频；保存后会自动刷新候选人状态与完成时间，并将音频落库。【F:src/main/java/com/example/grpcdemo/controller/JobCandidateController.java†L47-L66】【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L334-L409】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewRecordRequest.java†L9-L93】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewAudioRequest.java†L9-L71】
- **获取 AI 评估报告**：`GET /api/enterprise/job-candidates/{jobCandidateId}/ai-evaluation`
  - 返回 `CandidateAiEvaluationResponse`，含整体得分、评分等级、`strengths[]`、`weaknesses[]`、`riskAlerts[]`、`competencyScores{}`、`customMetrics{}`、`positionId`、`positionName` 以及原始 `rawPayload`，用于渲染第四张图展示的报告页面。【F:src/main/java/com/example/grpcdemo/controller/JobCandidateController.java†L54-L57】【F:src/main/java/com/example/grpcdemo/service/CompanyJobCandidateService.java†L376-L454】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateAiEvaluationResponse.java†L9-L108】
...
## Candidate interview portal（候选人面试门户）

- **左侧邀约列表**：`GET /api/candidate/interview-sessions/{jobCandidateId}/invitations`
  - 查询参数：`status`（`ALL`/`WAITING`/`IN_PROGRESS`/`COMPLETED`/`ABANDONED`/`CANCELLED`/`TIMED_OUT`）与 `keyword`（按岗位或企业名称模糊查询）。
  - 响应 `CandidateInterviewInvitationListResponse`，包含卡片数组 `invitations[]`（企业名称、岗位、HR 联系方式、候选人邮箱、头像元数据与下载地址、面试状态、预检/头像完成度、答题截止时间、失败原因等）以及 `statusCounters[]`（顶部标签统计），`total` 和 `activeStatus` 直接用于渲染分页与高亮。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L50-L56】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L141-L182】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewInvitationItem.java†L9-L168】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewStatusCounter.java†L9-L29】
- **右侧详情与提醒**：`GET /api/candidate/interview-sessions/{jobCandidateId}` 返回 `CandidateInterviewDetailResponse`，包含最新面试记录快照、预检状态、技术检查项列表、头像上传状态以及 `answerDeadlineAt`（开始答题后 +60 分钟，未点击“开始录音”时为 `null`）；`readyForInterview=true` 时才允许进入面试房间。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L58-L61】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L185-L219】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewDetailResponse.java†L9-L63】
- **麦克风/网络预检**：`POST /api/candidate/interview-sessions/{jobCandidateId}/precheck`
  - 请求体 `CandidateInterviewPrecheckRequest` 携带检测项 `requirements[]` 与 `summary`（通过与否、完成时间等）。
  - 后端写入 `candidate_interview_records.precheck_*` 字段并将候选人状态调整为 `SCHEDULED`，响应与详情一致用于刷新提示。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L64-L69】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L223-L260】【F:src/main/java/com/example/grpcdemo/entity/CandidateInterviewRecordEntity.java†L24-L118】
- **进入面试房间**：`POST /api/candidate/interview-sessions/{jobCandidateId}/start`
  - 可选参数：`locale`（题面语言）、`context{}`（业务上下文）以及 `refreshQuestions` 控制是否重新向 AI 拉题。
  - 返回 `CandidateInterviewStartResponse`，包含 `interviewRecordId`、`aiSessionId`、题目列表与元数据，首次进入会写入 `roomEnteredAt`，但不会立即开始计时，`interviewStartedAt` 与 `answerDeadlineAt` 可能为空；若记录已被判定超时将直接返回 410（`GONE`）。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L71-L75】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L263-L332】【F:src/main/java/com/example/grpcdemo/service/RestInterviewQuestionClient.java†L19-L86】
- **开始作答（点击“开始录音”）**：`POST /api/candidate/interview-sessions/{jobCandidateId}/begin-answering`
  - 无需请求体。后端会确认头像已上传且仍在答题窗口内，然后写入/保持 `interview_started_at` 并根据剩余时长刷新 `answer_deadline_at`，同时把候选人状态切换为 `IN_PROGRESS`。多次调用可在非正常中断后继续计时，响应 `CandidateInterviewBeginResponse` 带回最新 `interviewRecordId`、`interviewStartedAt` 与 `answerDeadlineAt` 供倒计时使用。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L71-L80】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L333-L362】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewBeginResponse.java†L1-L29】
- **中断/恢复计时**：`POST /api/candidate/interview-sessions/{jobCandidateId}/pause`
  - 前端在 `beforeunload`、断网心跳等场景调用该接口即可把计时器冻结，后端会记录最新 `answerElapsedSeconds`、`answerPausedAt` 并清空 `answerDeadlineAt`，返回的 `CandidateInterviewBeginResponse` 可用于继续展示已开始的时间轴。重新进入页面后再次调用 `begin-answering` 会从剩余时间续播。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L82-L85】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L366-L385】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewRecordResponse.java†L12-L227】
- **头像上传**：`POST /api/candidate/interview-sessions/{jobCandidateId}/profile-photo`，请求体 `CandidateInterviewProfilePhotoRequest` 携带 Base64 图片、文件名与 MIME。头像写入成功后 `CandidateInterviewProfilePhotoDto` 会返回下载地址；未上传头像时 `submitAnswer` 会直接返回 409 阻止作答。预览使用 `GET /profile-photo`。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L106-L140】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L559-L632】
- **题目作答与语音上传**：`POST /api/candidate/interview-sessions/{jobCandidateId}/answers`
  - 请求体 `CandidateInterviewAnswerRequest` 需携带题号、音频 Base64、时长、前端通过语音转写得到的文本 `transcript` 以及扩展 metadata。后端会先确认头像已上传并且已经执行过 `begin-answering`，随后在 60 分钟窗口内校验是否超时、自动生成音频 ID、覆盖旧音频并维护转写 JSON，然后返回 `CandidateInterviewAnswerResponse`（带最新面试记录及当前音频 DTO），方便第三张图同步刷新波形与字幕。面试记录中会同步累计 `answerElapsedSeconds`、`answerPausedAt`、`answerResumedAt` 与 `timedOut` 标记，便于前端展示“已用时/暂停时间/是否超时”。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L87-L91】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L388-L520】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewAnswerRequest.java†L16-L58】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewRecordResponse.java†L12-L227】
- **完成/放弃面试**：
  - 完成：`POST /api/candidate/interview-sessions/{jobCandidateId}/complete`（可附带总时长与 metadata）。在答题时限内调用会把状态设为 `COMPLETED`；若接口被调用时已经超时，后端会保留超时前的问答与音频、补齐结束时间并返回 `timedOut=true` 的 `CandidateInterviewRecordResponse`，前端应据此提示“作答已超时”。为了避免数据丢失，倒计时归零时也要自动调用一次该接口。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L93-L97】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L444-L520】【F:src/main/java/com/example/grpcdemo/controller/dto/CandidateInterviewRecordResponse.java†L12-L227】
  - 放弃：`POST /api/candidate/interview-sessions/{jobCandidateId}/abandon` 支持传入 `reason`，后端将候选人标记为 `ABANDONED` 并把原因写入记录 metadata，响应新的邀约卡片用于第五张图状态切换。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L99-L104】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L511-L555】
- **倒计时与强制结束**：候选人进入面试后若未在 `answerDeadlineAt` 之前上传全部答案，后端会自动把记录标记为 `TIMED_OUT`、补齐结束时间并在 `metadata.timeout` 中记录 `deadline`/`finalizedAt`。此时继续调用头像/作答接口会返回 410，但仍可调用 `complete` 获取已保存的问答快照给用户查看或提示重新预约。【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L223-L332】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L388-L520】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L884-L978】
- **音视频下载**：头像和题目音频分别通过 `GET /profile-photo` 与 `GET /audios/{audioId}` 提供二进制流，便于候选人或运营下载回放。【F:src/main/java/com/example/grpcdemo/controller/CandidateInterviewController.java†L106-L140】【F:src/main/java/com/example/grpcdemo/service/CandidateInterviewPortalService.java†L602-L632】
